...
#----------------------------------------------------------------------------#
# Name:        webserver.py                                                  #
# Purpose:     Simple webserver - webserver                                  #
# Author:      hzchenkj                                                     #
# Email:       hzchenkj@gamil.com                                           #
# Created:     2014/11/28                                                    #
# Copyright:   (c) 2014                                                      #
#----------------------------------------------------------------------------#
...
#read config file
try:
    configfilename=os.path.dirname(pubutil.cur_file_dir())+'/conf/webserver.conf'
...
class SecureHTTPServer(HTTPServer):
    def __init__(self, server_address, HandlerClass):
        ...
        self.server_bind() #banding
        self.server_activate() #激活
...
class ServerHandler(CGIHTTPRequestHandler):
        ...
        try:
            ...
            if cgi_moudle=="on" and self.path.endswith(cgi_extensions):
                ...
                if client_modified==file_last_modified:
                    self.send_response(304)  #304 缓存，不更新
                    self.end_headers()
                else:
                    if gzip=="on": #压缩
    ...
    def list_directory(self, path):
        try:
            ...
            return None
        list.sort(lambda a, b: cmp(a.lower(), b.lower()))#不区分大小写对目录列表排序
        f = StringIO()
